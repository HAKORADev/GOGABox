...
- [x] **1.1 THE DESIGN FOLLOWS THE CONTENT LAW** — root kill of the vertical
...
- [x] **1.2 PC = KEEP + BOX-BROWN, everywhere** — on PC the stretch aspect is
...
- [x] **1.3 THE EDGE VEIL LAW (the Spotify-like theming)** — when bars exist,
...
- [x] **1.4 THE FULLSCREEN RECALC LAW** — fullscreen flips re-apply the design
  on a fresh frame (never mid-transition), the stretched→fullscreen→restore
  scale bug dies with the content-driven design.
- [x] **1.5 THE SHARPNESS LAW** — canvas default filter = Linear Mipmap +
...
- [x] **1.6 DYNAMIC SCALE (the FSR law)** — settings Graphics toggle, OFF by
...
- [x] **1.7 THE ICON LAW** — a real multi-size .ico (16-256) built from the
  logo, wired as the windows native icon: taskbar / task manager / window
  all wear the SAME face.
- [x] **1.8 F10 + THE POSITION TOGGLE** — F10 flips the MENU position
  (vertical/horizontal), main menu only (never in-game), also a row in
  Screen settings, persisted.
- [x] **1.9 THE ESC LAW** — ESC = the phone back button, 1:1 (pause game →
  close sheet → leave box).
- [x] **1.10 THE ARROW LAWS** — Up/Down scroll the feed; at the top,
...
- [x] **1.11 THE FOCUS NUKE** — buttons never take keyboard focus; Tab and
  arrows can never walk between buttons (the unwanted focus ring family).
- [x] **1.12 THE GAMEPAD SEAT** — dpad = arrows, A/B/X/Y = keys 1-4, Start =
  ESC; wired and advertised for 4 games (snake, rally, invaders,
  brickbreaker) to start; the box never breaks without a pad.
- [x] **1.13 THE GOGACURSOR** — golden pixelated cursor, brown outlines,
...
- [x] **1.14 THE UNFOCUS PAUSE LAW** — app loses focus (both platforms):
...
- [x] **1.15 THE SPLASH FLICKER KILL** — the v010-era feed-flick: the whole
  ...
  only the logo fades.
- [x] **1.16 THE DOOMSCROLL LAW** — the old 64-78px banner reserve at the
...
- [x] **1.17 THE SETTINGS REWORK (AAA shape)** — SETTINGS = [AUDIO] (music +
...
- [x] **1.18 PLATFORM TAGS BACK** — the os tag rides every pre-play page
  header again (it lived in search + guide only).
- [x] **1.19 THE CONTROLS TAGS** — per-game control-scheme chips (touch /
  mouse+keyboard / gamepad) with icons in the tag-icon style, on the
  pre-play page + guide.
- [x] **1.20 THE GUIDE REWORK** — HOW TO PLAY first (universal wording), then
  CONTROLS split into TOUCH / GAMEPAD / MOUSE+KEYBOARD with the new icons —
  never platform-branded (a phone can get a gamepad).
- [x] **1.21 THE TAP-ANYWHERE LAW** — one universal full-screen tap overlay
...
- [x] 2.1 thumbnail = a code-programmed in-game capture
- [x] 2.2 tap anywhere uses the universal overlay (2.1's law)
- [x] 2.3 PC: RMB switches the shot, LMB fires, the idol follows the cursor
- [x] 2.4 ghost-chain preview = a moving sparkle line, never real marbles
- [x] 2.5 the jaw/hole layering: marbles ride OVER the jaw; jaw under the
  mouth arch, head over — the current jaw-over-hole render is wrong
- [x] 2.6 THE POP-BACK LAW: after a pop, the part TOWARD the hole rolls BACK
  to meet the rear part (faster), the rear stays; rolling marbles keep
  their colors
- [x] 2.7 the rolling looks like rolling: marbles rotate along the path
  direction (up-down over the chain), not the left-right self-spin
- [x] 2.8 THE LIVING INSERT: the shot physically slides in — the forward
  part is pushed one spacing over time while the shot sneaks in; nothing
  teleports
- [x] 2.9 pop VFX wears each popped marble's color
- [x] 2.10 powerup marbles: icon in the center only, normal fixed color,
  match-popped like any marble (no rainbow any-shot magnet)
- [x] 2.11 VAPOR: a majestic marble — the hit removes the chain with a
  staggered dissolve (matcher-style), never an instant clear
- [x] 2.12 the shooter never loads an extinct color: when a color leaves the
  level, the loaded/next marbles re-roll into existing colors on the fly
- [x] 2.13 the dev cheats sheet re-opens without restarting the app

## 3. GOLD MINER (v040-15 report)

- [x] 3.1 thumbnail = a code-programmed in-game capture (no more mud shot)
- [x] 3.2 the "tap anywhere" intro shows NO floating miner in the sky
- [x] 3.3 the miner rig stands ON the land surface (not sunk)
- [x] 3.4 nothing crosses the screen edges — the swing/field clamps to the
  visible walls (no reeling things from out-of-resolution ground)
- [x] 3.5 rocks pay NEGATIVE: -2/-4/-6 by size (gold stays +1/+2/+3)
- [x] 3.6 rocks ARE traps: generator places them as real danger around gold
  (bombs included), not decoration

## 4. THE SMALL-GAMES ROUND

- [x] 4.1 Fruit Slasher — slash particles no longer flash at the top-left
- [x] 4.2 DOMINO — the 0/0 (double) tile sits PERPENDICULAR to the line
- [x] 4.3 Heavy War — PC: arrows drive the tank, the mouse aims and shoots
- [x] 4.4 Cosmic Spud — arrows drive the ship; the belly bars and head
  bubbles are gone
- [x] 4.5 Space Invaders — PC: the ship follows the cursor, LMB shoots
- [x] 4.6 THE FIXED STEERING LAW — finger-steering games (brick breaker,
...
- [x] 5.1 built per the pending.md laws (submode, open/closed walls, big
...
- [x] 6.1 version v041 everywhere (config/projects.json both spots, exe
  preset stamp, any in-app print)
- [x] 6.2 probes + flow_test green; window_probe extended for the new laws
- [x] 6.3 thumbnails captured for marble + goldminer (and fourline check)
- [x] 6.4 docs truth: RESOLUTION_RULE + AGENTS.md + CI.md wear the new laws
- [x] 6.5 CI stays 0-warnings (Node 24 majors + ubuntu-24.04 already pinned
  in v040-17 — verify the workflow files)
- [x] 6.6 push; both builds green; the owner gets the numbered report

---

## SHIP NOTE (the round's truth, eye-verified)

- window_probe 26/0 (Xvfb): boot portrait, re-window both kinds, fullscreen
  flip + persistence, THE CONTENT LAW (fullscreen keeps the PORTRAIT design,
  KEEP, bars exactly when off-aspect, the governor never flips), and the new
  settings seat (SCREEN & GRAPHICS wears DISPLAY / POSITION / DYNAMIC SCALE
  / GOGACURSOR; CONTROLS lists the box keys).
- flow_test ALL TESTS PASSED (final run on the full v041 tree).
- dev-sheet reopen probe (tests/v041_dev_reopen.gd): 3 rounds of open /
  flip / close clean - the owner's v040-14/15 restart seat is verified
  healthy on v041 (device report will confirm).
- Vision pass (films/v041/): the menu at 9:16 (the doomscroll shade reads),
  the SQUARES 3-board game clean at the 9:16 canvas (the stretch class is
  dead), the goldminer capture with the rig ON the surface.
- Thumbnails: marble + goldminer are code-programmed in-game captures
  (tests/v041_thumbs*.gd + the crop step), per the owner's order.
