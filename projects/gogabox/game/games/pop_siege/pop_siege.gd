...
func _build_wall() -> void:
        # v0.3.8-8 THE WALL LAW (the owner: "what you did is wrong, try to
        # roll back the game code to pre patch 6, because the fix is wrong
        # ... the feel i want is to see that thing really behind the
        # interface, like it's really under it normally without that
        # blur-like thing you did"):
        # the v0.3.8-6 SHORE is ROLLED BACK whole - its fade strips were the
        # "blur-like thing": a wash that ate the field's own first rows and
        # columns AND sank flying bloons under a gradient. What replaces it
        # is the honest, normal thing: two SOLID opaque wall panels - the
        # top strip under the HUD row, the right dock under the folk panel
        # - seated BETWEEN the field and the interface widgets in the tree
        # (field's props, bloons, bullets, fx all render under them; the
        # panel + the HUD CanvasLayer render above). A tall tree or a wide
        # prop now walks UNDER a real surface and is simply GONE - the way
        # an interface normally sits in front of a world. No fade, no wash,
        # no field pixel ever covered: the walls start EXACTLY where the
        # field ends. Their tone is the same room brown the margins already
        # wear (the host backdrop #241407), so the occlusion edge is
        # seamless - the interface zone reads as one surface. The walls sit
        # still while the field shakes (the +/-6 shake margin pads their
        # outer sides only), and they never sleep with the night tint -
        # they are interface (the night law's own words).
        var vp := get_viewport_rect().size
        var room := Color("241407")
        var top := ColorRect.new()
        top.color = room
        top.position = Vector2(-14.0, -14.0)
        top.size = Vector2(vp.x + 28.0, FIELD.y + 14.0)
        top.mouse_filter = Control.MOUSE_FILTER_IGNORE
        add_child(top)
        var dock := ColorRect.new()
        dock.color = room
        dock.position = Vector2(FIELD.x + COLS * CELL, -14.0)
        dock.size = Vector2(vp.x - (FIELD.x + COLS * CELL) + 14.0, vp.y + 28.0)
