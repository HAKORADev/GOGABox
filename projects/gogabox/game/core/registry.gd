...
const GAMES := [
    ...
    {"id": "plays_t2", "title": "The Table's Resident", "desc": "Play 40 runs", "tier": 2, "rule": {"k": "stat", "key": "plays", "v": 40}},
        ...
        "needs_games": 11},
                ...
                "controls": [
                        "tap anywhere to open the menu, then DEPLOY (or the SCRAP SHOP, or the SHOP)",
                        "LEFT half of the screen: touch and slide - the tank rolls with your thumb (small slide = slow, big slide = fast); moving never aims",
                        ...
                        "machine guns (when bought) fire straight up on their own; rocket pods (when bought) launch one rocket per cell from the bottom hull, each tracking its OWN target",
                        "the top-left widget: hull, weapon slots, LVL + XP, and REMAINING - the wave clock and the enemies left; the scrap counter rides bottom-left, the bank lives in the shops",
                        "every XP orb is one point - level up mid-run and pick 1 of 3 boost cards; the back button never closes the cards",
                        "every scrap piece is one scrap - it banks when the run ends; the SCRAP SHOP spends the bank: wheels, hull, cannon, auto-loader, MG barrels, rocket pods and their racks; the SHOP sells skins for GOGACoins and locks them until bought",
                        "10 waves per place, a BOSS after every 10th wave (watch for the WARNING), then the tank drives through the tunnel into the next place - after place 10 the war loops, harder",
                        "a GOGACoin drops after every 500 kills - bosses pay 5; the score bonus is the score divided at the box gate",
                ],
