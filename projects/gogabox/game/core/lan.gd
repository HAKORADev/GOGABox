...
func _ident() -> Dictionary:
	...
	return {"name": LanProfile.player_name(), "pfp": LanProfile.pfp(),
			"role": LanProfile.role(), "anchor": LanProfile.anchor_short()}
...
func _add_local_seat(local_slot := 0) -> void:
	...
	var seat := {"dev": my_dev(), "name": String(idn.get("name", "")),
		"pfp": int(idn.get("pfp", 0)), "role": String(idn.get("role", "gamer")),
		"anchor": String(idn.get("anchor", "")),
		"seat": seats.size() + 1, "local_slot": local_slot,
		"platform": MY_PLATFORM, "state": _my_state}
	seats.append(seat)

## COMBO CO-OP: the host seats a second LOCAL player beside the remotes.
func add_local_slot() -> bool:
	if not is_host:
		return false
	for s in seats:
		if String(s.get("dev", "")) == my_dev() + "-p2":
			return false
	if seats.size() >= SEAT_CAP:
		return false
	var seat := {"dev": my_dev() + "-p2",
		"name": LanProfile.sanitize_name(LanProfile.player_name() + " II"),
		"pfp": (LanProfile.pfp() + 3) % LanProfile.PFP_VARIANTS,
		"role": "gamer", "anchor": LanProfile.anchor_short(),
...
func remove_local_slot() -> bool:
	if not is_host:
		return false
	for i in seats.size():
		if String(seats[i].get("dev", "")) == my_dev() + "-p2":
			seats.remove_at(i)
			_reseat()
			_broadcast_seats()
			session_changed.emit()
...
func send_act(a: Dictionary) -> void:
	if _match_game == "" or not session_active():
		return
	var msg := {"t": "act", "game": _match_game, "who": my_seat_no(), "a": a}
	if is_host:
		for dev in _conns:
			_send_to(_conns[dev], msg)
