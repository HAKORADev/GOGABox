extends Node
## dario_probe - v0.3.1: drives CURSED DARIO headless. The owner's laws:
## the expensive entry (100), the /10 bonus, the ten maps (all parse, all
## carry a start and a door, the tenth carries the Witcher), the tile
## physics (Dario lands, jumps), the stomp table, the blocker shield (3
## stomps), the death law (-200, restart, 3 lives), the door law, the
## boss law (20 stomps, +100, the ending), the night theme, and the
## cursed replay counter.
##
...
func _run() -> void:
        Box.reset_all()
        print("== dario_probe: CURSED DARIO v0.3.1 ==")
        ...
        for i in D.LEVELS.size():
                ...
                var has_start := false
                for r in map.size():
                        if String(map[r]).length() != w:
                                even = false
                        if String(map[r]).contains("D"):
                                has_door = true
                        if String(map[r]).contains("d"):
                                has_start = true
                var name_ok := even and has_door and has_start
                _check(name_ok, "level %d parses: %d rows, even %s, door+start %s/%s"
        ...
        # ---- boot: the run starts, the playthrough is counted ----
        ...
        # ---- the stomp table: one stomp = one kind's points ----
        g.set_score(0)
        ...
        for ee in g.enemies:
                if String(ee["kind"]) == "snail":
                        snails += 1
        g._stomp(e)
        _check(int(g.score) == 10, "a snail stomp pays exactly +10")
        ...
        _check(int(g.score) == 10 + 25, "the blocker pays +25 when it finally falls")
        ...
        # ---- the door: walking into it advances the level ----
        g2_setup(g)
        await get_tree().process_frame
        var lv2: int = int(g.level_i)
        for it in g.items:
                if String(it["kind"]) == "door":
                        g.p_node.position = g._cell_center(it["cell"])
                        break
        g._reach_door()
        await get_tree().create_timer(4.0).timeout
        _check(int(g.level_i) == lv2 + 1, "the door opens the NEXT level")

        # ---- the boss: 20 stomps, +100, the ending ----
        var g3: GogaGame = D.new()
        g3.game_id = "dario"
        add_child(g3)
        await get_tree().process_frame
        while int(g3.level_i) < 9:
                g3.level_i = int(g3.level_i) + 1
        g3._load_level(9)
        g3._start_boss(Vector2i(9, 6))
...
func g2_setup(g: GogaGame) -> void:
        # a fresh level-1 state for the door law (the death test reloaded it)
        g._load_level(0)
