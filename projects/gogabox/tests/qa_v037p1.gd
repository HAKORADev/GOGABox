extends Node
## qa_v037p1 - the v0.3.7-1 Xvfb shot driver (the owner's "many visual
## tests" round). Rigs (QA_RIG env):
##  tower_geo    - GEOMETRIC style: the inner squares FIT the platforms,
##                 the coin is the REAL GOGACoin, the flip cube
...
##  snake_endless- the big world + the camera + the fruit compass arrow
##  cs_gore      - a bomb blast: gibs + blood + a fire pool + charred
##
##  QA_RIG=<rig> xvfb-run -a godot --path . res://tests/qa_v037p1.tscn
...
func _ready() -> void:
        ...
        if rig.begins_with("tower"):
                ...
                await _maze(rig)
        elif rig.begins_with("snake"):
                await _snake(rig)
        elif rig.begins_with("cs"):
                await _cs(rig)
        await _settle(7)
        var shot := "user://qa_v037p1_%s.png" % rig
        var img := get_viewport().get_texture().get_image()
        img.save_png(shot)
        print("[qa_v037p1] saved ", shot)
        get_tree().quit(0)
...
func _tower(rig: String) -> void:
        ...
        if G.has_method("_apply_geo_style"):
                G._apply_geo_style()
        G.phase = "run"
        # walk the cube right so the flips + the tail read (long enough to
        # cross a platform and leave a ribbon)
        for i in 90:
                G._set_axis(0.95)
...
# ---------------------------------------------------------------- snake
func _snake(rig: String) -> void:
        await _boot("snake", 1920, 1080)
        if not Box.unlock_owned("snake", "endless"):
                Box.buy_unlock("snake", "endless", 0)
        Box.set_progress("snake", "enemy_count", 4)
        Box.set_progress("snake", "opt_enemies", true)
        G._show_mode_select()
        await _settle(2)
        # pick endless through the real card callback path
        G.endless_mode = true
        G.wrap_mode = true
        G._apply_field_size()
        G._show_ready_card()
        ...
        await _settle(3)
        G._populate_world()
        # let the world tick: the fruit pops, the camera settles, the
        # enemies roam far (the law) - then a long wait so the arrow reads
        for i in 120:
                G._goga_tick(1.0 / 60.0)
                await _settle(1)
