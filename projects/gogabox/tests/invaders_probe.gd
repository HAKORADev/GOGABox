...
func _start_run() -> void:
	...
	await _frames(2)
	G._dialog_dismiss()          # the intro lore popup
	await _frames(2)
	G._dialog_dismiss()          # the stage line popup
	await _wait(1.5)
...
func _run() -> void:
	...
	# ---- boot ----
	await _boot()
	_check(G.phase == "ready", "boots to the ready card (no ask screens)")
	...
	_check(G.weapon_level() == G.POWER_MAX, "the ladder caps at 5 (max a weapon)")
	...
	# ---- titan: same damage to EVERY enemy ----
	G.weapon = "missile"
	...
	_check(int(t1["hp"]) == th1 - 2 and int(t2["hp"]) == th2 - 2,
	...
	_check(G.bg_a.texture == G._tex["bg_mars"], "the pack paints each world its own sky")
