...
func _build_menu() -> void:
        ...
        # NOTE: order matters! custom_minimum_size must be set BEFORE position.
        # With anchors CENTER_TOP, setting position while size is still (0,0)
        # bakes offset_right=-300, and min-size expansion then grows the rect
        # from the anchor point -> title rendered half off-screen (x=420).
        var title := _label("JELLY", 110, ACCENT)
        title.custom_minimum_size = Vector2(600, 130)
        title.position = Vector2(60, 200)
        menu.add_child(title)
        var title2 := _label("JUMP", 110, Color.WHITE)
        title2.custom_minimum_size = Vector2(600, 130)
        title2.position = Vector2(60, 320)
        ...
        _menu_coins.position = Vector2(0, 990)
        ...
        _sim_banner.position = Vector2(0, 1230)
        _sim_banner.size = Vector2(720, 50)
        _sim_banner.mouse_filter = Control.MOUSE_FILTER_IGNORE
        var bl := _label("BANNER AD (simulated on desktop)", 20, Color(1, 1, 1, 0.5))
